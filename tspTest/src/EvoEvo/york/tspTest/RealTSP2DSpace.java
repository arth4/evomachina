...
public class RealTSP2DSpace extends Toroidal2DSpace implements SearchableSpace {
    ...
    private Map<Journey2DSite, RealJourney> _replicants;

    /** Construct the most likely situation with defined x and y size and no containing space: */
    public RealTSP2DSpace(int xSize, int ySize) {
    ...
    private void findReplications(Journey2DSite s) {
        ...
        if (neighbours.size() == 1) {
            synchronized (_replicants) {
                _replicants.put(s, (RealJourney)neighbours.stream().findAny().get().getASubspace());
            }
            ...
            RealJourney bestMatch = (RealJourney)neighbours
                    .stream()
                    .map(sp -> sp.getASubspace())
                    .min((j1, j2) -> ((Individual)j1).compareTo((Individual)j2))
                    .get();
    ...
    public  void replicateInto(Site target, Individual parent) {
        synchronized (target) {
            RealJourney replicant = (RealJourney)parent.replicate();
